            ...
            <div>
                ...
                if (!empty($get_list))
                {
                    ...
                    foreach ($get_list as $res)
                    {
                        ...
                        echo                '<td class="service_' . $res["id"] . '">' . date("d-m-Y H:i",strtotime($res["date"])) . '</td>';
                        echo                '<td class="service_' . $res["id"] . '">' . $res["amount"] . '</td>';
                        echo                '<td class="service_' . $res["id"] . '">' . $res["state"] . '</td>';
                        if($res["state"] == "Validée")
                        {

                        }
                        else
                        {
                            echo            '<td> <button class="button" onclick="valid_custo_order('. $res["id"] .')">Valider</button> ';
                            echo            ' <button class="button" onclick="delete_custo_order('. $res["id"] .')">Annuler</button> </td>';
